    ...
    >
      <Link href="/">
        ...
        <div className="relative max-h-full w-full rounded-2xl">
          ...
          />
          <motion.div
            className="absolute right-1/2 top-1/2 z-10 flex -translate-y-1/2 translate-x-1/2 items-center justify-end gap-3 rounded-full bg-white/15 
          py-2 pl-5 pr-2 font-semibold backdrop-blur-md"
            variants={{
              rest: { opacity: 0, filter: 'blur(16px)', width: '56px' },
              hover: { opacity: 1, filter: 'blur(0px)', width: '170px' }
            }}
          >
            <motion.p className="absolute left-5 z-0">Ver detalhes</motion.p>
            <motion.div
